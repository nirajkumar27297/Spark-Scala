...
object TumblingWindowDemoWithWaterMark {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val valueDF = kafkaSourceDF.select(
      from_json(col("value").cast("string"), stockSchema).alias("value")
    )
    ...
    val tradeDF = valueDF
      ...
      .withColumn(
        "Sell",
        expr("case when Type == 'SELL' then Amount else 0 end")
      )
    ...
    val outputDF =
      windowAggDF.select("window.start", "window.end", "TotalBuy", "TotalSell")

    outputDF.printSchema()
    val windowQuery = outputDF.writeStream
      .format("console")
      .outputMode("update")
      .option("checkpointLocation", "chk-point-dir-watermark")
