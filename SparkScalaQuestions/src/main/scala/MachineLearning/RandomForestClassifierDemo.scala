...
object RandomForestClassifierDemo extends App {
  def createSession(): SparkSession = {
    val spark = SparkSession
      .builder()
      .master("local[*]")
      .appName("RandomForestClassifierDemo")
      .getOrCreate()
    spark
  }
  ...
  indexedDF.show()
  val split = indexedDF.randomSplit(Array(0.80, 0.20))
  val train = split(0)
  val test = split(1)
  val randomForestModel = new RandomForestClassifier()
    ...
    .setSeed(111)
  val predictions = randomForestModel.fit(train).transform(test)
  ...
  println(s"The accuracy  is %1.2f%%".format(accuracy * 100))
  spark.stop()
